...
function ListaReservas() {
  ...
  const eliminar = async (id: number) => {
    const confirmar = window.confirm("¿Seguro que querés eliminar esta reserva?");
    if (!confirmar) return;
      ...
      <ul>
        {reservas.map((reserva) => (
          <li key={reserva.id}>
            ...
            <strong>{reserva.salon.nombre}</strong> reservado por {reserva.usuario.nombre} — {reserva.fecha.slice(0, 10)}{" "}
            de {reserva.horaInicio} a {reserva.horaFin} — Estado: {reserva.estado}
